...
class ItemButtom extends Button:
	var item
...
func _new_item_button(item) -> Button:
	var result = ItemButtom.new(item, self)
...
#	scroll_container.add_child(inner_container)
	...
	inner_container.size.x = maxLen * 10 + 15
	inner_container.size.y = lines * 10 + 15
